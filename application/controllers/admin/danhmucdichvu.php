...
class danhmucdichvu extends Admin_controller {
    ...
    public function index() {
        ...
        $data = array();
        $data['render_path'] = array('Admin' => base_url() . 'admin/trangchu/home', 'Cảm nhận học viên' => base_url() . 'admin/danhmucdichvu-ad/home');
        $data['heading_title'] = 'Quản lý cảm nhận học viên';
                ...
                if (gettype($del) == 'array' && count($del) > 0) {
                    ...
                    foreach ($del as $id) {
                        ...
                        if ($this->danhmucdichvu->delete($id)) {
                            $this->session->set_flashdata('warning', 'Xóa cảm nhận học viên thành công');
                        } else {
    ...
    public function add_edit() {
        ...
        $data['render_path'] = array('Admin' => base_url() . 'admin/trangchu/home', 'Cảm nhận học viên' => base_url() . 'admin/danhmucdichvu-ad/home');
        $data['heading_title'] = 'Tạo - Cập nhật cảm nhận học viên';
        $data['action'] = base_url() . 'admin/danhmucdichvu-ad/add_edit';
        ...
        if ($this->form_validation->run() == TRUE) {
            ...
            if ($id && $id != '') {
                ...
                if ($this->danhmucdichvu->update($id, $data)) {
                    $this->session->set_flashdata('warning', 'Cập nhật cảm nhận học viên thành công');
                ...
                if ($this->danhmucdichvu->add($data)) {
                    $this->session->set_flashdata('warning', 'Thêm mới cảm nhận học viên thành công');
                    redirect('admin/danhmucdichvu-ad/home');
                } else {
                    $this->session->set_flashdata('warning', 'Có lỗi rồi');
                    redirect('admin/danhmucdichvu-ad/add_edit');
    ...
    function delete() {
        ...
        if ($this->danhmucdichvu->delete($id)) {
            ...
            $this->session->set_flashdata('warning', 'Xóa cảm nhận học viên Thất bại!');
            redirect('admin/danhmucdichvu-ad/home');
        }
        //}
    }

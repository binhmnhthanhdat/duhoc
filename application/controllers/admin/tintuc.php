...
class Tintuc extends Admin_controller {
    ...
    public function index() {
        ...
        $data['article'] = $this->tintuc->get_all_news(array('id', 'title', 'img', 'img_big', 'description', 'content',   'id_group_new',   'display',   'alias',   'metakeyword', 'metadescription', 'ord', 'modified', 'active',   'post_date'), $title, $des, $tag, $news_group, $project_group, $status, null, array('id' => 'asc'), array('max' => $config['per_page'], 'begin' => $page));
        ...
        $sql = "select * from cat_new where active=1 order by ord asc";
    ...
    public function add_edit() {
        ...
        if ($_id != '')
            $data['article'] = $this->tintuc->get_by_id($_id);
        $sql = "select * from cat_new where active=1 order by ord asc";
